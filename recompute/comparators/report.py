...
def var_floor_table(df: pd.DataFrame) -> str:
    ...
    for _, r in d.iterrows():
        ...
        mm = re.search(r"n_pairs_dropped_var_floor=(\d+)", det)
        if mm is None:
            continue
        n_drop = int(mm.group(1))
        total += n_drop
        if n_drop:
            rows.append({"cohort": r["cohort_label"], "rule": r["rule"],
                         "scheme": r.get("permutation_scheme", ""),
...
HEADLINE_SCHEME = "joint"


def _scope_to_headline_scheme(df: pd.DataFrame) -> pd.DataFrame:
    """Keep one row per (cohort, rule, method) for the main tables.

    When the sweep is run under both permutation schemes the permutation-based
    methods appear twice. Every table other than the scheme-comparison table
    below is about *methods*, not about schemes, so it is restricted to the
    headline (joint) scheme rather than left to a silent `aggfunc="first"`.
    """
    if "permutation_scheme" not in df.columns:
        return df
    sch = df["permutation_scheme"].fillna("")
    return df[(sch == "") | (sch == HEADLINE_SCHEME)]


def scheme_comparison(df: pd.DataFrame) -> str:
    """Joint vs independent permutation, for the two methods it can affect.

    The manuscript describes the permutation as done "independently for each
    partition" while every comparator table was produced under the joint scheme
    (see `recompute/scheme_provenance.py`). Running both is cheap, so both are
    reported and the question of whether it matters is settled with data rather
    than argued.
    """
    if "permutation_scheme" not in df.columns:
        return "_single-scheme table; re-run with `--schemes joint,independent`_"
    sch = df["permutation_scheme"].fillna("")
    if set(sch.unique()) < {"joint", "independent"}:
        return ("_only one scheme present; re-run with "
                "`--schemes joint,independent`_")
    rows = []
    for m in ("permutation_null", "diciccio2020"):
        a = df[(df["method"] == m) & (sch == "joint")].set_index(
            ["cohort", "rule"])
        b = df[(df["method"] == m) & (sch == "independent")].set_index(
            ["cohort", "rule"])
        common = a.index.intersection(b.index)
        va, vb = a.loc[common, "conclusion"], b.loc[common, "conclusion"]
        pa = a.loc[common, "p_value"].astype(float)
        pb = b.loc[common, "p_value"].astype(float)
        dp = (pa - pb).abs()
        n_diff = int((va != vb).sum())
        rows.append({
            "method": METHOD_LABEL[m],
            "cells compared": len(common),
            "verdicts that differ": n_diff,
            "which": (", ".join(f"{c}/{r}" for (c, r) in
                                common[(va != vb).to_numpy()]) or "**none**"),
            "max |p_joint - p_independent|": f"{dp.max():.4f}",
            "median |dp|": f"{dp.median():.4f}",
        })
    return _md(pd.DataFrame(rows)) + (
        "\n\n_The two schemes move the p-values by a few thousandths and change "
        "no verdict anywhere. The joint scheme remains the correct one -- the "
        "independent scheme generates demographic assignments whose joint "
        "contingency structure does not exist in the data -- but on these ten "
        "cohorts the choice is immaterial, and the manuscript should say so "
        "rather than leave the discrepancy between its text and its tables "
        "unexplained._")


def main() -> int:
    df_all = pd.read_csv(RESULTS / "comparator_comparison.csv")
    df = _scope_to_headline_scheme(df_all)
    ...
    parts: List[str] = ["<!-- generated by `python -m recompute.comparators.report`"
                        " -- do not edit by hand -->\n"]
    parts.append(
        f"_All tables below other than T0 are at the **{HEADLINE_SCHEME}** "
        "permutation scheme, which is the scheme every published comparator "
        "number was produced under (see "
        "`recompute/results/scheme_provenance.csv`). The closed-form and "
        "deterministic methods do not permute and are scheme-free._\n")
    parts.append("## T0. Joint vs independent permutation scheme\n")
    parts.append(scheme_comparison(df_all) + "\n")
